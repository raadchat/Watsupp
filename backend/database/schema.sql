...
-- ملاحظة (المرحلة 10 — رسائل واتساب): profile_name (اسم العرض في واتساب،
-- من حقل contacts[].profile.name في كل Webhook وارد — قد يتغيّر، فيُحدَّث
-- لأحدث قيمة معروفة في كل رسالة) يُضاف أيضاً عبر ترقية تلقائية.
