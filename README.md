...
### ي) إخفاء رقم العميل عن الوكيل (المرحلة 7)

في "إعدادات البوت": مفتاح "إظهار رقم العميل في محادثات خدمة العملاء"
(افتراضياً مفعَّل، ليطابق السلوك الحالي قبل هذه المرحلة). عند إيقافه، يرى
Agent (وليس Admin — هو يرى الرقم دائماً) رقماً مموَّهاً
(`••••••1234`، آخر 4 خانات فقط) في طابور الانتظار، محادثاتي النشطة، وعند
فتح المحادثة — في `backend/utils/customerPresentation.js` حصراً، طبقة عرض
فقط تُطبَّق على استجابة الـ API مباشرة قبل إرسالها. **الرقم الحقيقي الكامل
يبقى في قاعدة البيانات دون أي تغيير**، ويُستخدَم كما هو تماماً في كل مكان
آخر غير هذه الاستجابات (إرسال/استقبال رسائل واتساب الفعلي، Webhook، الربط
بالعميل الصحيح) — الإخفاء عرضي بحت.

### ك) سجلات الوكلاء (المرحلة 8)

لكل Agent (زر 📊 في "المستخدمون"، لا يظهر لـ Admin — عرض للمدير فقط):

**سجل الدخول/الخروج** (`agent_login_logs`) — صف جديد عند كل تسجيل دخول
ناجح (`authController.login`)؛ الخروج يُسجَّل فقط عند ضغط زر "تسجيل الخروج"
صراحةً (نداء جديد `POST /api/logout`) — إغلاق المتصفح أو انتهاء الجلسة
بعد 24 ساعة يبقيان بلا خروج مسجَّل، ويظهران كذلك في الواجهة عمداً، تماماً
كما وصفت. مُجمَّع حسب اليوم في الاستجابة نفسها.

**سجل العملاء** (`customer_service_sessions`، جدول جديد مستقل عن
`customers` — الذي يعكس آخر حالة فقط ولا يحتفظ بتاريخ الجلسات السابقة):
صف واحد لكل دورة طابور→تولٍّ→إنهاء كاملة، يُحدَّث تلقائياً من نفس نقاط
التحوّل الموجودة فعلاً (طلب خدمة العملاء، claim، end، رد التقييم) — بلا أي
مسار جديد يوازيها. عدد الرسائل يُحسَب مباشرة من `messages` ضمن نافذة
[دخول الانتظار، انتهاء المحادثة] لكل جلسة، لا عمود منفصل قد يختلّ تزامنه.
التقييم هنا خاص بهذه الجلسة تحديداً؛ `admins.rating_total/rating_count`
(الموجود مسبقاً) يبقى كما هو مجموعاً تراكمياً — هذا الجدول يُغذّيه بجانبه،
ولا يستبدله. ⚠️ ملاحظة دقة بسيطة: توقيت SQLite هنا بدقة الثانية، فحدثان
يقعان في نفس الثانية بالضبط قد يُحتسبان معاً عند حدود نافذة جلسة — أمر غير
وارد عملياً بين تفاعلات بشرية حقيقية (تولٍّ ثم رد فعلي يستغرقان أكثر من
ثانية دائماً)، ولوحظ فقط أثناء اختبار آلي فائق السرعة.

### ل) تعدد محادثات خدمة العملاء + تحديث حي (المرحلة 9)

**Socket.IO** أُضيف للمشروع لأول مرة (لم يكن موجوداً إطلاقاً، تأكَّدت من هذا
في تحليل المرحلة 0) — يلتف حول نفس خادم Express (لا منفذ ثانٍ). كل اتصال
يُصادَق بنفس JWT المستخدَم في REST، وينضمّ فوراً لغرفة خاصة بمعرّفه
(`user:{id}`). عند وصول رسالة عميل أثناء محادثة نشطة (`CUSTOMER_SERVICE_ACTIVE`)
ومُسنَدة لوكيل: `customers.unread_count` (عمود جديد) يزداد، ويُبَث حدث
`conversation:new-message` (يحمل `conversationId`, `customerId`, `message`,
`timestamp`, `unreadCount` بالضبط كما حدَّدت — بلا أي بيانات إضافية غير
ضرورية) لغرفة ذلك الوكيل **فقط**، لا بثاً عاماً. كل محادثة مستقلة تماماً
ولا قيد `currentConversation` وحيداً في أي مكان — راجع `webhookController.js`.

**في `agent.js`:** قائمة "محادثاتي" تعرض 🔴 لكل محادثة فيها رسائل غير
مقروءة، تُحدَّث حيّاً بلا Reload. فتح محادثة يصفّر عدّادها فقط (عبر
`customersController.getCustomerMessages` المُحدَّثة)، ولا يمسّ البقية.
رسالة تصل لمحادثة **مفتوحة بالفعل** تُضاف مباشرة داخل اللوحة (بلا إعادة
جلب كاملة) وتُصفَّر فوراً (`POST /api/customers/:id/mark-read`، نقطة جديدة
خفيفة لا تجلب الرسائل كاملة). رسالة تصل لمحادثة **غير مفتوحة** لا تنقل
الشاشة إليها إطلاقاً — فقط يزداد عدّادها في القائمة. الاستطلاع الدوري
(كان كل 5 ثوانٍ) أصبح كل 15 ثانية كشبكة أمان فقط (تغطية الطابور، وإعادة
مزامنة كاملة لو انقطع الاتصال الحي مؤقتاً) — التحديث الفعلي أصبح حيّاً.

**⚠️ حدود الاختبار في بيئتي — مهم لتقييم هذه المرحلة بدقة:**
بيئتي بلا إنترنت، فلا أقدر أُثبّت حزمة `socket.io` الحقيقية لأختبر اتصال
WebSocket فعلياً. ما اختبرته آلياً وبدقة (18 حالة، كلها نجحت): كل منطق
القرار في `webhookController.js` — متى يُزاد العدّاد، متى يُبَث الحدث ومتى
لا (لا بث لعميل بالطابور أو في تدفق البوت العادي)، صحة محتوى الحدث بالكامل،
واستهداف غرفة الوكيل **الصحيح تحديداً** (اختبرت وكيلين مختلفين وتأكدت أن كل
حدث يصل لغرفته هو فقط)، واستقلالية كل محادثة عن الأخرى، وتصفير محادثة واحدة
بلا مسّ غيرها. **ما لم أختبره فعلياً** (يحتاج تشغيلك الحقيقي بعد
`npm install`): الاتصال الحي نفسه عبر متصفح حقيقي — بروتوكول WebSocket
والمصادقة عبره فعلياً.

**خطوات الاختبار الإجباري الذي وصفته بالضبط (بعد `npm install && npm start`):**
1. افتح `agent.html` في نافذتين (أو متصفحين)، سجّل دخول نفس الوكيل في الأولى فقط.
2. من واتساب، اجعل 3 أرقام مختلفة (A, B, C) تطلب "خدمة العملاء"، وتولَّهم كلهم من نافذة الوكيل.
3. من واتساب: أرسل من A رسالة، من B رسالة، من C رسالة، ثم من A رسالة ثانية — دون تحديث الصفحة.
4. المتوقَّع: تظهر 🔴 على B وC فوراً في القائمة (2 على A لاحقاً)، بلا نقل الشاشة، وبلا أي Reload.
5. افتح محادثة B من القائمة — يُصفَّر عدّادها فقط، ويبقى عدّاد A/C كما هو.
6. من واتساب: أرسل من A رسالة جديدة — يجب أن يظهر إشعار 🔴 جديد على A فوراً.

إن لم تنجح هذه الخطوات بالضبط، أخبرني بالتفصيل (أيّ خطوة فشلت، ورسالة أي خطأ
في Console المتصفح إن وجدت) لأصحّحها فوراً في نسخة مُراجَعة من هذه المرحلة.

### م) صفحة "رسائل واتساب" (المرحلة 10)

صفحة جديدة في **لوحة الأدمن تحديداً** (Sidebar، كما وصفت) — منفصلة عمداً عن
شات الوكيل (`agent.js` يبقى كما هو من المرحلة 9). تعرض **كل** عميل لديه أي
رسائل (وليس فقط محادثات خدمة العملاء)، مرتَّبين بآخر نشاط، مع اسم واتساب
الفعلي إن توفر (`customers.profile_name`، عمود جديد يُلتقَط تلقائياً من
حقل `contacts[].profile.name` في كل Webhook وارد — لم يكن يُلتقَط سابقاً).

القائمة (`customersRepository.findConversations`) تُظهر معاينة آخر رسالة
فعلياً (نصية أو نوع مرفق 🖼️/🎥/📄) ووقتها وعدّاد غير المقروء، عبر subquery
واحد بلا تكرار بيانات. المحادثة نفسها **تعيد استخدام** نفس الدوال الموجودة
أصلاً (`renderConversationThread`، `api.sendCustomerMessage` بمرفقاته من
المرحلة 2) بدل بناء نظام موازٍ — الفرق فقط عرضها بلوحة inline بدل Modal.

**الجوال:** لوحة واحدة في كل مرة (القائمة أو المحادثة)، مع زر "→ رجوع" خاص
بهذه الصفحة تحديداً للعودة للقائمة — لا علاقة له بزر الرجوع داخل قوائم
واتساب نفسها (المرحلة 1). **سطح المكتب:** القائمة والمحادثة جنباً لجنب دائماً.

تستخدم نفس Socket.IO من المرحلة 9 (لا اتصال ثانٍ) لتحديث المحادثة المفتوحة
هنا فوراً لو وصلت لها رسالة أثناء عرضها، دون انتظار فتح/إغلاق يدوي.

**⚠️ نفس حدود الاختبار الصادقة كالمرحلة السابقة:** اختبرت آلياً وبدقة (10
حالات، كلها نجحت) استعلام القائمة بالكامل (الترتيب، المعاينة، البحث،
استبعاد من لا رسائل له) والتقاط اسم واتساب. **الواجهة نفسها** (تخطيط
سطح المكتب/الجوال، عمل زر الرجوع فعلياً، ووصول التحديث الحي لهذه الصفحة
تحديداً) تحتاج تجربتك المباشرة بعد `npm install && npm start` — دقّقت
يدوياً في كل معرّف HTML المستخدَم في الكود الجديد للتأكد من تطابقها
جميعاً حرفياً (كنت وقعت في خطأ مشابه سابقاً في المرحلة 6)، لكن هذا تدقيق
كودي وليس تجربة متصفح فعلية.
