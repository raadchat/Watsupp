...
**(المرحلة 5)** عند أول تشغيل، إن لم يوجد أي مستخدم باسم `admin` تحديداً،
يُنشأ تلقائياً حساب `admin` / `admin` (بكلمة مرور bcrypt hash، ليست نصاً
صريحاً أبداً). **⚠️ غيّر كلمة المرور فوراً بعد أول دخول**، من "المستخدمون"
في اللوحة. لن يُنشأ حساب مكرَّر إن كان `admin` موجوداً بالفعل (بهذا الاسم
تحديداً، سواء الافتراضي أو مُعدَّلاً).

تريد حساباً أول باسم مختلف بدلاً من ذلك؟ استخدم السكربت التفاعلي:
...
يطلب اسم مستخدم **موجود بالفعل** وكلمة مرور جديدة (6 أحرف على الأقل)،
ويحدّث الـ hash المخزَّن فقط — لا تُطبَع أي كلمة مرور في الطرفية أبداً.
...
### ز) الرجوع في قوائم واتساب (المرحلة 1)

كل قائمة تفاعلية (أقسام أو خدمات) **غير الجذر** تُظهر صفاً أول ثابتاً
"↩️ رجوع" (`id: BACK`، محجوز أيضاً كـ `category_id`/`service_id` — لا يمكن
لمدير إنشاء قسم أو خدمة بهذا المعرّف). الضغط عليه يعيد المستوى السابق تماماً
بخطوة واحدة (وليس الجذر مباشرة)، عبر مسار كامل محفوظ لكل عميل
(`customers.navigation_stack`، JSON لمعرّفات الأقسام من الجذر حتى الموضع
الحالي) — يعمل تلقائياً مع أي عمق تداخل مستقبلي بلا أي تعديل إضافي. لا يظهر
في القائمة الرئيسية إطلاقاً، ولا يُعيد تشغيل المحادثة أو يفقد أي سياق آخر
للعميل. لوحة التحكم والصفحات الإدارية (Dashboard/Agent/Settings/Users) غير
معنيّة بهذا التنقل إطلاقاً — خاص بقوائم واتساب فقط.

### ح) النصوص والأزرار المركزية (المرحلة 4)

كل نص/زر ثابت موجَّه للعميل على واتساب (تقريباً 25، من رسائل الأقسام
والخدمات إلى خدمة العملاء والتقييم والإشعارات) مُعرَّف بمفتاح ونص افتراضي
في `backend/services/botTexts.js`، وقابل للتعديل من صفحة "النصوص والأزرار"
في اللوحة (Admin فقط). أي تخصيص يُخزَّن في جدول `bot_texts` (مفتاح/قيمة) —
حقل فارغ يُعيد ذلك النص لافتراضيه تلقائياً. لا كاش: أي تعديل يسري فوراً في
الرسالة التالية بلا إعادة تشغيل الخادم. تدعم بعض النصوص استبدال ديناميكي
مثل `{agentName}` وقت الإرسال فقط (استبدال نصي بسيط، لا تنفيذ كود من النص
بأي شكل). **مهم:** المُعدَّل هنا هو النص الظاهر فقط — أي payload/id ثابت في
الكود (`BACK`، `__customer_service__`، `rating_1..5`، `notif_yes`/`notif_no`)
لا يتغيّر أبداً ولا علاقة له بهذا النظام، فتغيير أي نص لا يكسر منطق البوت.
رسالة الترحيب تبقى عمداً في مكانها الحالي (`bot_settings.welcome_message`،
مع صورتها) ولم تُنقَل لهذا الجدول — لها بالفعل مكان تعديل واحد، ونقلها كان
سيُنشئ تكراراً بدل حذفه.

### ط) إدارة المستخدمين (المرحلة 6)

صفحة "المستخدمون" تدير admin وagent معاً الآن (كانت مقصورة على agent فقط) —
إضافة، تعديل الاسم/النوع، حذف، وتغيير كلمة مرور أي مستخدم من زر 🔑 منفصل.
اسم المستخدم غير قابل للتعديل بعد الإنشاء. الصلاحيات مُطبَّقة بالكامل في
الخادم (`usersController.js`)، وليس فقط بإخفاء الأزرار: مسار كامل مقصور على
`role='admin'`، ولا يمكن لأي مستخدم حذف أو تخفيض حسابه الخاص أثناء تسجيل
دخوله به، ولا حذف/تخفيض **آخر** مدير في النظام مهما كان الحساب المنفِّذ —
يمنع قفلاً كاملاً للوحة بالخطأ. حذف مستخدم مرتبط بعملاء سابقين
(`customers.assigned_agent_id`) أو رسائل جماعية أنشأها (`bulk_jobs.created_by`،
كلاهما Foreign Key حقيقي) يُصفِّر تلك الإشارات فقط قبل الحذف — لا يفشل
الحذف، ولا يُحذف أي سجل محادثة أو رسالة فعلي (`messages.sent_by` نص ثابت
غير متأثر إطلاقاً).
...
### المرحلة 3 — إعدادات ENV الذكية

`JWT_SECRET` لم يعد مطلوباً في `.env` — إن غاب، يُولَّد تلقائياً وبأمان
(`crypto.randomBytes(32)`) عند أول إقلاع ويُخزَّن بشكل دائم في
`system_settings` (لا يتغيّر بعدها، وإلا انتهت صلاحية كل الجلسات عند كل
إعادة نشر)؛ ولا يُعرض هذا السرّ أو يُسجَّل في أي مكان مطلقاً. تصنيف كل
متغيرات `.env.example`: `PORT`/`NODE_ENV` (توفّرهما منصة التشغيل)،
`BCRYPT_SALT_ROUNDS`/`JWT_EXPIRES_IN`/حدود المعدّل/`BULK_MESSAGE_DELAY_MS`
(قيم افتراضية آمنة، لا حاجة لضبطها)، بيانات واتساب (مُدارة فعلياً من صفحة
"الاتصال بواتساب" — مكان إدخال واحد فقط، مموَّهة دائماً في أي استجابة API).
أيضاً: أي خطأ غير متوقَّع (استثناء برمجي، فشل شبكة هارب) يُسجَّل في السيرفر
عبر `message`/`stack` فقط (`utils/errors.js: logSafeError`)، وليس الكائن
الخام بخصائصه بالكامل — يمنع تسريب رؤوس مثل `Authorization` لو حملها الخطأ
عرَضاً.
